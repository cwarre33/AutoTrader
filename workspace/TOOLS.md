...
All commands run via: `python workspace/tools/alpaca_tool.py <command> [args]`
...
```bash
python workspace/tools/alpaca_tool.py account
```
Returns: equity, buying_power, cash, portfolio_value, day_trade_count

### `positions` — List all open positions
```bash
python workspace/tools/alpaca_tool.py positions
```
...
```bash
python workspace/tools/alpaca_tool.py bars AAPL,MSFT,NVDA --days 30
```
